...
class ClaimClassifier:
    ...
    def predict(self, texts):
        ...
        mapping = {0: 'dependent-claim', 1: 'independent-claim'}
        # df = pd.DataFrame({'predicted_labels': predicted_labels,
        #                    'probabilities': probabilities})
        df = pd.DataFrame({'predicted_labels': predicted_labels})
        df['predicted_labels'] = df['predicted_labels'].map(mapping)
        predicted_labels = df['predicted_labels'].tolist()
        return predicted_labels

    def eval(self):
        train1, test1, train2, test2, train3, test3 = train_test0824()
        texts = test1[0].tolist()
        mapping = {0: 'dependent-claim', 1: 'independent-claim'}
        test1[1] = test1[1].map(mapping)
        trues = test1[1].tolist()
